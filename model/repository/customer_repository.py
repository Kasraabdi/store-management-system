...
class CustomerRepository:
    ...
    def save(self, customer: Customer):
        with self.get_connection() as conn:
            ...
            if customer.code:
                cursor.execute("""
                    INSERT INTO customers (code, name, family, username, password, phone_number, locked)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (
                    int(customer.code), customer.name, customer.family,
                    customer.username, customer.password, customer.phone_number,
    ...
    def edit(self, customer: Customer, original_code=None):
        target_code = original_code if original_code else customer.code
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                UPDATE customers
                SET code = ?, name = ?, family = ?, username = ?, password = ?, phone_number = ?, locked = ?
                WHERE code = ?
            """, (
                int(customer.code), customer.name, customer.family,
                customer.username, customer.password, customer.phone_number,
                1 if customer.locked else 0, int(target_code)
            ))
            conn.commit()
            return customer
    ...
    def delete(self, code):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("DELETE FROM customers WHERE code = ?", (int(code),))
            conn.commit()
    ...
    def find_by_code(self, code):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT code, name, family, username, password, phone_number, locked FROM customers WHERE code = ?", (int(code),))
            row = cursor.fetchone()
            if row:
