...
def name_validator(name):
    if name is not None:
        ...
        if not (len(cleaned) >= 2 and re.match(pattern, cleaned)):
            raise ValueError("نام وارد شده نامعتبر است (باید بین ۲ تا ۳۰ کاراکتر باشد).")
...
def family_validator(family):
    if family is not None:
        ...
        pattern = r"^[a-zA-Z\u0600-\u06FF\uFB50-\uFDFF\uFE70-\uFEFF\u200C\s]{2,30}$"
        if not (len(cleaned) >= 2 and re.match(pattern, cleaned)):
            raise ValueError("نام خانوادگی وارد شده نامعتبر است (باید بین ۲ تا ۳۰ کاراکتر باشد).")
...
def phone_number_validator(phone_number):
    if phone_number is not None and phone_number != "":
        ...
        if not re.match(r"^09\d{9}$", clean_phone):
            raise ValueError("شماره همراه نامعتبر است (باید ۱۱ رقم بوده و با ۰۹ شروع شود).")
